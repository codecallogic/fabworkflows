    ...
    >
      ...
      >
        <div className="form-group">
          <input
          value={manageFormFields(autoFill.account, 'name')} 
          onChange={(e) => (stateMethod(createType, 'account', e.target.value), setSearch(e.target.value))}/>
          <label 
          className={`input-label ` + (
            autoFill.account.length > 0 || 
            typeof autoFill.account == 'object' 
            ? ' labelHover' 
            : ''
          )}
          htmlFor="account">
        ...
        <div className="addFieldItems-modal-form-container-searchList-container">
          <div className="addFieldItems-modal-form-container-searchList-table-header">
            <div 
              ...
              onClick={() => sort === 'down' ? setSort('up') : setSort('down')}
            >
              <span>Account</span>
          ...
          <div className="addFieldItems-modal-form-container-searchList-list">
            { allData && allData.accounts.sort((a, b) => sort === 'down' ? a.name > b.name ? -1 : 1 :  a.name > b.name ? 1 : -1).map((item, idx) => 
              search.length > 0 ? 
